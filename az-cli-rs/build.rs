...
fn emit_clap_types(services: &[(String, aaz_gen::model::ServiceModule)]) -> String {
    let mut out = String::new();
    ...
    for (cli_prefix, svc) in services {
    ...
    // Emit nested enums for each service
    for (cli_prefix, svc) in services {
        let prefix = capitalize(cli_prefix);
        // If the service has exactly one group, merge it directly
        if svc.groups.len() == 1 && !svc.groups[0].subgroups.is_empty() {
...
fn emit_dispatch_fn(services: &[(String, aaz_gen::model::ServiceModule)]) -> String {
    let mut out = String::new();
    ...
    // Generate per-service dispatch functions
    for (cli_prefix, svc) in services {
        let variant = capitalize(cli_prefix);
        let svc_mod = svc.name.replace('-', "_");
        out.push_str(&format!("async fn dispatch_{svc_mod}(cmd: {variant}Commands, _svc: &str) -> crate::error::Result<Option<serde_json::Value>> {{\n"));
        out.push_str("    match cmd {\n");
        if svc.groups.len() == 1 && !svc.groups[0].subgroups.is_empty() {
